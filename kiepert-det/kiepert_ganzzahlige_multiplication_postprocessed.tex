...
From $\sigma u$ we derive the function $\wp u$ by differentiation, namely
\begin{align*}
\frac{d\log \sigma u}{du} = \frac{\sigma' u}{\sigma u} &= \frac{\sigma'}{\sigma}(u)\footnote{For brevity we write $\frac{\sigma'}{\sigma}(u)$ instead of $\frac{\sigma' u}{\sigma u}$.} = \frac{1}{u} + \sideset{}{'}\sum \left(\frac{1}{u-w} + \frac{1}{w} + \frac{u}{w^2}\right),\\
...
\end{equation}
possesses both periods $2\omega$ and $2\omega'$, as soon as $\Sigma a = \Sigma b$ is, because upon substitution
\begin{equation*}
...
\end{equation*}
then it results, when we develop both sides of equation (11.),
\begin{equation*}
e_1 + e_2 + e_3 = 0,
\end{equation*}
thus
...
\begin{equation}
\begin{cases}
\psi_n(u) = C \dfrac{\sideset{}{'}\prod \left[\sigma\left(u - \dfrac{2\lambda\omega + 2\mu\omega'}{n}\right) e^{(2\lambda\eta + 2\mu\eta')\frac{u}{n}}\right]}{(\sigma u)^{nn-1}}\\begin{equation*}1.5em]
...
To investigate this determinant $D_{n-1}(v)$, we substitute the variable $u$ for the specific value $v$ for which it vanishes; then it follows first that $D_{n-1}(u)$ is an entire function of $\wp u$ and $\wp' u$, since the higher derivatives of $\wp u$ are entire functions of $\wp u$ and $\wp' u$. If we now replace $u$ by $-u$, only the odd derivatives of $\wp u$ change their sign, while the even ones remain unchanged. This gives, for
\begin{equation*}
...
To determine the degree of these functions, we seek the first term in the expansion of $D_{n-1}(u)$ in powers of $u$, which we obtain by substituting into $D_{n-1}(u)$ only the first term of the expansion of $\wp' u$, $\wp'' u$, \ldots\ This yields the determinant\footnote{The dagger symbol in the original indicates a footnote or annotation.}
\begin{equation*}
\begin{vmatrix}
